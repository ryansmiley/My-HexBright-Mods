  ...
  case MODE_LOW:
    if (btnDown && !newBtnDown && (time-btnTime)>30){
      ...
      if (millis()-lastChangeMode < 900)
        ...
        newMode = MODE_OFF;
      pastMode = MODE_LOW;
    } else if (btnDown && newBtnDown && (time-btnTime)>300){
//      Serial.println("Button Long Press");
//      usePastMode = 1;
//      newMode = MODE_OFF;
//      mode = MODE_OFF;
    } else if (btnDown && newBtnDown && (time-btnTime)>1000){
      newMode = MODE_STROBE_PREVIEW;
      pastMode = MODE_LOW;
    }
    break;
...
//      mode = MODE_OFF;
  ...
  if (newMode != mode)
  {
    ...
    case MODE_OFF:
      ...
      digitalWrite(DPIN_DRV_EN, LOW);
      if (usePastMode == 1){
        if (StoredEEuse != usePastMode){
          Serial.print("Writing to EEPROM 3: ");
          EEPROM.write(3,usePastMode);
        }
